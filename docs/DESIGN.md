...
| ボイス=GPT Realtime(4つ目の選択肢) | Worker が OpenAI Realtime API(`REALTIME_API_BASE_URL` = `https://api.openai.com/v1` の `/realtime` を Workers の `fetch` + `Upgrade: websocket` で WebSocket 化。vars `REALTIME_MODEL`/`REALTIME_VOICE`)に外向き接続し、PCM16 24kHz の音声デルタを集めて WAV に変換してから既存 `POST /tts` のプロトコルで返す(`{"engine":"realtime"}`) | 端末に鍵や WebSocket クライアントを載せない(#61 の TLS ヒープ枯渇再燃を回避、DESIGN.md 3 章の「API キーは Worker 秘匿」方針を維持)。端末側の再生経路(WAV 全量バッファ→`playWav`)は無変更。Realtime 失敗(エラー・15 s タイムアウト・`response.done` 前の切断)時は部分音声を捨てて Worker が従来 TTS に、それも失敗すれば端末がチャープにフォールバックし無音にしない(端末側の `/tts` タイムアウトは Realtime 要求時のみ 45 s) |
