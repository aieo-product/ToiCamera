...
| PLA フィラメント | 約 10g |
| **M2 × 12mm タッピングネジ × 2** | ウォッチの元ネジ(M2×8 実測)を置き換える。**実機で 12mm がちょうど良いことを確認済み** |
...
2. プレートを背面に当て、**M2×12 で共締め**(締めすぎ注意 — 樹脂ボスです)
3. プレートの向き: **穴のない広い側がボタン・スピーカー側**。スピーカーやボタンには一切かかりません

![duo プレートを M2×12 で装着した状態](images/plate-mounted.jpg)
...
![カメラ単体を背面中央に装着](images/camera-center.jpg)

### カメラ + GPS — 下段に縦向きで並べる

下段の **左右 2 ペア(各 16mm)** に 1 台ずつ。両ユニットを縦向きで横に並べます(GPS はユニット自身の 2 穴にピンを直挿しでも OK)。

![カメラと GPS を下段に縦向きで装着](images/duo-units.jpg)

横から見るとこのくらいの厚みです(握りやすい奥行きになります):

![側面から見た装着状態](images/side-view.jpg)
